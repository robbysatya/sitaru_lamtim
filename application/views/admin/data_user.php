<div class="wrapper">
				...
				<div class="row mb-2">
					<div class="col-sm-6">
						<div class="row">
							<div class="col-sm-6">
								<h1><?= $title; ?></h1>
								<a href="<?= base_url('admin/Data_User/Tambah_User') ?>" class="btn btn-primary mt-3"><i class="fas fa-fw fa-plus"></i> Tambah User</a>
							</div>
						</div>
					</div>
				</div>
											...
											<?php foreach ($data_user as $user) :  ?>
												...
												<td>
													<a href="<?= base_url('admin/data_user/edit_user/' . $user['id']) ?>" class="btn btn-warning row-sm-8" style="color: #fff;"><i class="fas fa-fw fa-edit"></i>
														Edit</a>
													<button class="btn btn-danger row-sm-8" data-toggle="modal" data-target="#deleteDataUserModal<?= $user['id']; ?>"><i class="fas fa-fw fa-trash"></i>
														Hapus</button>
												</td>
										</tr>
										<?php $no++ ?>
	...
	<?php foreach ($data_user as $user) : ?>
		<div class="modal fade" id="deleteDataUserModal<?= $user['id']; ?>" tabindex="-1" aria-labelledby="deleteDataDosenLabel" aria-hidden="true">
				...
				<div class="modal-content">
					...
					<form action="<?= base_url('admin/data_user/Hapus_User') ?>" method="post">
						<div class="modal-body">
							...
							<input type="text" class="form-control" id="id" name="id" placeholder="ID" required value="<?= $user['id']; ?>" hidden>
						</div>
