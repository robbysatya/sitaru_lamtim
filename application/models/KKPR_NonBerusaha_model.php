...
class kkpr_NonBerusaha_model extends CI_Model
{
	...
	public function simpan()
	{
		...
		$config['upload_path'] = './assets/file/kkpr_nonberusaha';
		...
		if (!empty($_FILES['file_ktp_pemohon'])) {
			$this->upload->do_upload('file_ktp_pemohon');
			$data2 = $this->upload->data();
			$file_ktp_pemohon = $data2['file_name'];

			$this->db->set('file_ktp_pemohon', $file_ktp_pemohon);
		}
		...
		if (!empty($_FILES['file_dokumen_perizinan_sebelumnya'])) {
			...
			$file_dokumen_perizinan_sebelumnya = $data11['file_name'];

			$this->db->set('file_dokumen_perizinan_sebelumnya', $file_dokumen_perizinan_sebelumnya);
		}
		if (!empty($_FILES['file_kepentingan_umum'])) {
			$this->upload->do_upload('file_kepentingan_umum');
			$data12 = $this->upload->data();
			$file_kepentingan_umum = $data12['file_name'];

			$this->db->set('file_kepentingan_umum', $file_kepentingan_umum);
		}
		...
		$this->db->insert('tb_kkpr_nonberusaha');

		$this->db->set('status_nonberusaha', 'Tunggu');
		$this->db->set('alasan_nonberusaha', '');
		$this->db->set('no_dokumen_nonberusaha', '');
	...
	{
		$data['kkpr_belum'] = $this->db->get_where('tb_kkpr_nonberusaha', ['nik' => $nik])->row_array();

		$file_permohonan_bermaterai = $data['kkpr_belum']['file_permohonan_bermaterai'];
		$file_ktp_pemohon = $data['kkpr_belum']['file_ktp_pemohon'];
		...
		$file_kepentingan_umum = $data['kkpr_belum']['file_kepentingan_umum'];
		...
		unlink(FCPATH . 'assets/file/kkpr_nonberusaha/' . $file_permohonan_bermaterai);
		unlink(FCPATH . 'assets/file/kkpr_nonberusaha/' . $file_ktp_pemohon);
		unlink(FCPATH . 'assets/file/kkpr_nonberusaha/' . $file_bukti_penguasaan_tanah);
		unlink(FCPATH . 'assets/file/kkpr_nonberusaha/' . $file_surat_keterangan_waris);
		unlink(FCPATH . 'assets/file/kkpr_nonberusaha/' . $file_dokumen_perjanjian_sewa);
		unlink(FCPATH . 'assets/file/kkpr_nonberusaha/' . $file_dokumen_akta_jual_beli);
		unlink(FCPATH . 'assets/file/kkpr_nonberusaha/' . $file_surat_pernyataan_pemohon);
		unlink(FCPATH . 'assets/file/kkpr_nonberusaha/' . $file_polygon_koordinat);
		unlink(FCPATH . 'assets/file/kkpr_nonberusaha/' . $file_rencana_penggunaan_air);
		unlink(FCPATH . 'assets/file/kkpr_nonberusaha/' . $file_dokumen_perizinan_sebelumnya);
		unlink(FCPATH . 'assets/file/kkpr_nonberusaha/' . $file_kepentingan_umum);
		unlink(FCPATH . 'assets/file/kkpr_nonberusaha/' . $file_gambar_sisi_utara);
		unlink(FCPATH . 'assets/file/kkpr_nonberusaha/' . $file_gambar_sisi_selatan);
		unlink(FCPATH . 'assets/file/kkpr_nonberusaha/' . $file_gambar_sisi_barat);
		unlink(FCPATH . 'assets/file/kkpr_nonberusaha/' . $file_gambar_sisi_timur);

		$alasan =  $this->input->post('alasan');

		$this->db->set('status_nonberusaha', 'Ditolak');
		$this->db->set('alasan_nonberusaha', $alasan);
		$this->db->where('nik', $nik);
		$this->db->update('tb_user');
